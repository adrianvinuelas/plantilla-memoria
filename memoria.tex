...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Hasta este momento todavia no había incluido ni JSHint, ni Highcharts.
  \end{itemize}
...
Bootstrap es un framework o conjunto de herramientas de código abierto para dise\~{n}o de sitios y aplicaciones web. Contiene plantillas de dise\~{n}o con tipografía, formularios, botones, cuadros, menús de navegación y otros elementos de dise\~{n}o basado en HTML y CSS.

Para usar Bootstrap en una página HTML, el desarrollador solo debe descargar la hoja de estilo Bootstrap CSS y enlazarla en el archivo HTML. Permite dise\~{n}ar las aplicaciones web tanto para móviles,ordenadores o tablets sin tener que modificar la hoja de estilo CSS de nuestro proyecto, puesto que el estilo viene definido en una serie de clases. Solo con darle una clase a nuestro objeto este ya aplica sus propiedades,lo cual facilita mucho el trabajo.
...
jQuery-UI es una biblioteca de componentes para el framework jQuery que le a\~{n}aden un conjunto de plug-ins, widgets y efectos visuales para la creación de aplicaciones web. Cada componente o módulo se desarrolla de acuerdo a la filosofía de jQuery.
...
Bootstrap\footnote{\url{http://getbootstrap.com}}.
jQuery\footnote{\url{https://jquery.com/}}.
jQuery UI\footnote{\url{https://jqueryui.com}}.
Highcharts\footnote{\url{http://www.highcharts.com}}.
Django\footnote{\url{https://www.djangoproject.com}}.
AJAX\footnote{\url{http://api.jquery.com/jquery.ajax/}}.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{img/esquema}
...
\begin{itemize}
  ...
  \item el ranking de los 5 errores más comunes (rankingerrors),
...
\begin{itemize}
  ...
  \item la rama a la que pertenece el ejercicio (branch), puesto que en Github hay dos ramas en las que se pueden subir los ejercicios, la rama master (por defecto) o la rama gh-pages (que te crea un servidor en Github para ese ejercicio),
  ...
  \item el número identificativo para un mismo ejercicio (numorden), puesto que un usuario puede analizar un ejercicio las veces que quiera, por tanto hay que diferenciar cada análisis, ya que la información no se sobreescribe. Es decir, un usuario puede tener almacenados:
  \begin{itemize}
...
\begin{itemize}
  ...
  \item un resumen de los errores del alumno que solo utiliza el rol de profesor (sumary),
...
Este objeto contiene la información relacionada con el analisis de un fichero .js. Almacena los siguientes datos:
\begin{itemize}
  \item la url del ejercicio del profesor, en el caso de que el usuario tenga rol de profresor (exercise),
  ...
  \item el resumen con el tipo de errores y el número de errores de cada tipo (sumary),
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto4}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto6}
...
\begin{itemize}
  ...
  \item busca ficheros \textbf{.js}, cuando encuentra uno lo analiza, aplicando las herramientas JSLint y JSHint, identificando cada \textbf{error} y guardandolo en la base de datos,
...
\end{figure}
 \item Gráfica de resumen de errores de fichero:
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto8}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto9}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto15}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto16}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{img/foto17}
...
Cuando antes explicaba en el análisis de un ejercicio que guardabamos el resumen de los errores de un fichero, era porque lo ibamos a necesitar ahora para poder ver en que parte del código está localizado el error. El resumen (en el que se agrupan todos los errores según el tipo de error) de un fichero es el siguiente:
\begin{figure}[H]
    \centering
...
Es el servidor quien cuando crea los errores, almacena en la base de datos para cada error la url en la que se nos mostrará donde está el error en el fichero en Github, y por otra parte almacena también la solución a ese error. Posteriormente cuando crea el resumen del fichero, incluye unos \textbf{href} en el resumen, que al pusarlos son los que nos redirigen tanto a la solución como a la visualización del error. La solución de los errores está definida en una función que tiene el servidor en la que están definidos todos los tipo de error así como sus soluciones.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{img/ejerciciostotales}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{img/evolucioncurso}
...
\begin{itemize}
  \item Partimos de que al principio se parte de un número de 1911 errores. El primer ejercicio lo entregaron un total de 30 alumnos.
...
Esto se debe a que 8 alumnos no entregaron el ejercicio. La aplicación nos informa de ello. Por tanto, es lógico que el número de errores haya disminuido.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{img/ejer9y10}
...
En este caso es al revés. En el noveno, que es la rama \textbf{master}, lo entregaron 17 alumnos y en el décimo, que es la rama \textbf{gh-pages}, lo entregaron 24. Esto se debe a que hay ejercicios que se piden que sean entregados directamente en la rama gh-pages y por eso hay alumnos que directamente no entregan nada en la rama master y si en la rama gh-pages. Como se puede ver la diferencia de errores entre una rama y otra es de 1022 errores. Por tanto, no solo vale con analizar una rama en muchos casos, si no que analizar las dos nos puede decir con más exactitud que alumnos entregaron un ejercicio y que alumnos no. Además, estos dos ejercicios (que en realidad son uno pero separado en dos ramas) son una práctica importante del curso (y no un ejercicio de entrega normal), con lo cual el número de errores vuelve a subir porque lo han entregado la mayoría de los alumnos, y los ficheros suelen tener mucho más código. 
  \item Y para terminar la simulación podemos observar el top 5 de los errores más comunes a lo largo del curso. Estos errores pueden dar una idea de que puntos debe comentar el profesor en clase a sus alumnos. En nuestro caso, vemos que los errores más comunes son:
\begin{figure}[H]
  \centering
...
\begin{itemize}
  ...
  \item La aplicación permite ver la explicación del por qué salen los errores, es decir, unicamente no ves que identifica errores, si no que da la posibilidad al alumno de que entienda el por qué salen los errores y como solucionarlos.
